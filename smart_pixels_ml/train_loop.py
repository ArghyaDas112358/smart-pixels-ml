...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("runLOG.txt"), logging.StreamHandler()],
)
...
dataset_base_dir = (
    "/depot/cms/users/das214/datasets/dataset_3sr/dataset_3sr_16x16_50x12P5_parquets/"
)
...
tfrecords_base_dir = os.path.join(
    dataset_base_dir, "TFR_files", f"2t_N_{NOISE_MU}mu_{NOISE_SIGMA}sig_NoLog_Stdr"
)
...
class SoftQuantizeLoggerCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if not hasattr(layer, "num_levels"):
            logging.warning(
                f"Layer '{self.layer_name}' is not SoftQuantizeLayer; skipping log."
            )
            return
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if vloss > self.thr or not np.isfinite(vloss):
            ...
            if self.bad >= self.pat:
                print(
                    f"[AbortOnStuck] val_loss {vloss:.1f} ≥ {self.thr} "
...
def main(seed):
    ...
    checkpoint_filepath = os.path.join(
        checkpoints_dir, "weights.{epoch:02d}-t{loss:.2f}-v{val_loss:.2f}.hdf5"
    )
    logging.info(f"Checkpoints will be saved to: {checkpoint_filepath}")

    logging.info("Setting up Callbacks...")
    mcp = tf.keras.callbacks.ModelCheckpoint(
        filepath=checkpoint_filepath, save_weights_only=True, save_freq="epoch"
    )
    ...
    quantizer_logger = SoftQuantizeLoggerCallback(
        log_filepath=quantizer_log_path, layer_name="soft_quantizer_output"
    )
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logging.error(
                f"An exception occurred in main execution: {e}", exc_info=True
            )
