...
class LinearScheduler(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_loss > self.stop_thresh:
            ...
            print(
                f"[Epoch {epoch+1}] val_loss={val_loss:.1f} > {self.stop_thresh} → reg_weight={new_w:.4f}"
            )
        else:
            # If val loss is below or equal to the threshold, freeze λ at its current value
            print(
                f"[Epoch {epoch+1}] val_loss={val_loss:.1f} ≤ {self.stop_thresh} → reg_weight frozen at {self.reg_weight.numpy():.4f}"
            )
...
class AdaptiveScheduler(tf.keras.callbacks.Callback):
    def __init__(
        self, reg_weight_var, start=0.5, max_reg_weight=5.0, step=0.05, patience=15
    ):
        self.reg_weight = reg_weight_var
        self.start = start
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if nll < self.best_nll - 1e-2:
            ...
            self.wait = 0
            print(
                f"[Epoch {epoch+1}] NLL improved to {nll:.2f} → reg_weight stays at {reg_weight:.4f}"
            )
        else:
            self.wait += 1
            ...
            if self.wait >= self.patience and reg_weight < self.max_reg_weight:
                ...
                print(
                    f"[Epoch {epoch+1}] NLL plateaued  → reg_weight increased to {new_weight:.4f}"
                )
        ...
        if nll > self.best_nll + 0.5:
            ...
            self.reg_weight.assign(new_weight)
            print(
                f"[Epoch {epoch+1}] NLL worsened  → reg_weight reduced to {new_weight:.4f}"
            )
...
class CosineScheduler(Callback):
    def __init__(
        self, reg_weight_var, start=0.5, end=10.0, max_epochs=500, stop_threshold=-25000
    ):
        super().__init__()
        self.reg_weight = reg_weight_var
        self.start = start
