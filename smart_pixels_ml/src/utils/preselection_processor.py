...
def preselection_processing(pitch):
    data_directory_path = "/data/dajiang/smartPixels/dataset_3s/dataset_3sr_{}_parquets/unflipped/recon3D/".format(
        pitch
    )
    labels_directory_path = "/data/dajiang/smartPixels/dataset_3s/dataset_3sr_{}_parquets/unflipped/labels/".format(
        pitch
    )
    data_format = "3D"
    file_type = "parquet"
    ...
    labels_files = [
        ...
        + recon3D_file.split("/")[-1].replace("recon" + data_format, "labels")
        for recon3D_file in recon3D_files
    ]
    ...
    data_preselection_directory_path = "/data/dajiang/smartPixels/dataset_3s/dataset_3sr_{}_parquets/unflipped/recon3D_cotBeta1P5/".format(
        pitch
    )
    labels_preselection_directory_path = "/data/dajiang/smartPixels/dataset_3s/dataset_3sr_{}_parquets/unflipped/labels_cotBeta1P5/".format(
        pitch
    )
    ...
    for i in tqdm(
        range(len(joined_files)), desc="Processing files for {}".format(pitch)
    ):
        ...
        recon3D_df.to_parquet(
            data_preselection_directory_path + recon3D_filename.split("/")[-1]
        )
        labels_df.to_parquet(
            labels_preselection_directory_path + labels_filename.split("/")[-1]
        )
...
if __name__ == "__main__":
    ...
    print(
        "2. Applies preselection cuts to recon3D/labels files of matching file number"
    )
