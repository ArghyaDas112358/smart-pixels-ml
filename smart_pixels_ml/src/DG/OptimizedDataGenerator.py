...
class OptimizedDataGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_time_stamps = (
            np.arange(0, 20) if use_time_stamps == -1 else use_time_stamps
        )
        ...
        self.use_time_stamps = (
            np.array(
                [
                    np.arange(idx[i][0], idx[i][1]).astype("str")
        ...
        self.label_files = [
            labels_directory_path
            + recon_file.split("/")[-1].replace("recon" + data_format, "labels")
            for recon_file in self.recon_files
        ]
    ...
    def prepare_batch_data(self, batch_index):
        ...
        file_index = (
            np.arange(self.file_offsets.size)[index < self.file_offsets][0] - 1
        )  # first index is 0!

        index = index - self.file_offsets[file_index]  # relative event index in file
        batch_size = min(
            index + self.batch_size,
            self.file_offsets[file_index + 1] - self.file_offsets[file_index],
        )
        ...
        if file_index != self.current_file_index:
            ...
            if self.file_type == "csv":
                ...
                recon_df = pd.read_parquet(
                    self.recon_files[file_index], columns=self.use_time_stamps
                )
                labels_df = pd.read_parquet(
                    self.label_files[file_index], columns=self.labels_list
                )
        ...
        y = labels_df[index:batch_size] / np.array([75.0, 18.75, 8.0, 0.5])
