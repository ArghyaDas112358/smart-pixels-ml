...
class OptimizedDataGenerator(tf.keras.utils.Sequence):
    def __init__(
        ...
        **kwargs,
        ...
        if load_from_tfrecords_dir is not None:
            ...
            self.use_time_stamps = (
                list(np.arange(0, 20)) if use_time_stamps == -1 else use_time_stamps
            )
    ...
    def process_file_parallel(self):
        file_infos = [
            (afile, self.recon_cols, self.input_shape, self.transpose, self.labels_list)
    ...
    def save_batches_parallel(self):
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            future_to_batch = {
                executor.submit(self.save_single_batch, i): i
            ...
            for future in tqdm(
                ...
                total=num_batches,
                desc="Saving batches as TFRecords",
            ):
    ...
    def prepare_batch_data(self, batch_index):
        ...
        stop_idx = min(
            rel_idx + self.batch_size,
            self.file_offsets[file_idx + 1] - self.file_offsets[file_idx],
        )
        ...
        if file_idx != self.current_file_index:
            ...
            df = pd.read_parquet(parquet_file, columns=all_columns_to_read).reset_index(
                drop=True
            )

            # df = pd.read_parquet(parquet_file, columns=self.use_time_stamps)
            recon_df, labels_df = split_df_to_X_y_df(
                df, self.input_shape, self.labels_list, self.recon_cols
            )
        ...
        y = labels_df[rel_idx:stop_idx] / np.array([75.0, 18.75, 8.0, 0.5])
    ...
    def __getitem__(self, batch_index):
        ...
        for X_batch, y_batch in parsed_dataset:
            ...
            if self.quantize:
                X_batch = QKeras_data_prep_quantizer(
                    X_batch, bits=4, int_bits=0, alpha=1
                )
    ...
    def __len__(self):
        if (
            len(self.file_offsets) != 1
        ):  # used when TFRecord files are created during initialization
