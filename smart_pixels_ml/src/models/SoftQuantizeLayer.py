...
class SoftQuantizeLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            isinstance(n_bits, int) and n_bits > 0
    ...
    def _init_thresholds(self):
        ...
        if self.initial_thresholds is not None:
            arr = np.asarray(self.initial_thresholds, dtype=np.float32).astype(
                np.float32
            )
            assert (
                arr.shape[0] == B
            ), "initial_thresholds length must equal 2^n_bits - 1"
        else:
            lo, hi = self.initial_range
    ...
    def build_thresholds(self):
        ...
        deltas_thresholds_init = np.diff(
            initial_thresholds, prepend=self.threshold_offset
        )

        assert np.all(
            deltas_thresholds_init > 0
    ...
    def levels(self):
        ...
        if not self.trainable_levels:
            ...
            return tf.concat(
                [self.first_level, self.first_level + cumulative_deltas], axis=0
            )
...
if __name__ == "__main__":
    ...
    y_soft_initial = layer._soft_quantize(
        x_input, layer.k, layer.levels, layer.thresholds, layer.tau
    )

    (line_hard,) = ax.plot(
        x_input, y_hard_initial, "r-", lw=2.5, label="Hard Quantize (Forward Pass)"
    )
    (line_soft,) = ax.plot(
        x_input,
        y_soft_initial,
        ...
        alpha=0.8,
        lw=2.0,
        label="Soft Quantize (Backprop Approx.)",
    )

    vlines = ax.vlines(
        layer.thresholds.numpy(),
        -0.5,
        4.0,
        colors="g",
        lw=2,
