...
def evaluate(config):
    ...
    try:
        ...
        config["outFileName"] = Path(
            str(config["outFileName"]).replace("%j", str(job_env.job_id))
        )
    except:
        config["outFileName"] = Path(
            str(config["outFileName"]).replace("%j", "%08x" % random.randrange(16**8))
        )
    ...
    tfrecords_dir = Path(
        output_directory, f"tfrecords_{'%08x' % random.randrange(16**8)}"
    ).resolve()
    ...
    model = CreateModel(
        (13, 21, 2), n_filters=config["n_filters"], pool_size=config["pool_size"]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--inFolder", help="path to the training results", default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--njobs",
        help="number of jobs to actually launch. default is all",
        default=-1,
        type=int,
    )
    parser.add_argument(
        "--doOverwrite",
        help="overwrite any existing evaluation files",
        action="store_true",
    )
    ...
    top_dir = Path(
        inFolder, "eval", f'{"%08x" % random.randrange(16**8)}', "%j"
    ).resolve()
    ...
    for weightsFolder in weightsFolders:
        ...
        outFileName = Path(
            top_dir,
            weightsPath.parts[-3],
            weightsPath.parts[-1].replace(".hdf5", "_eval.csv"),
        ).resolve()

        if outFileName.exists() and not args.doOverwrite:
            print(
                f"Warning: {outFileName} exists. If you want to overwrite it pass in --doOverwrite."
            )
            continue
