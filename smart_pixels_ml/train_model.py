...
def train(
    ...
    try:
        ...
        output_directory = Path(
            str(output_directory).replace("%j", str(job_env.job_id))
        )
    except:
        output_directory = Path(
            str(output_directory).replace("%j", "%08x" % random.randrange(16**8))
        )
    ...
    tfrecords_dir_validation = Path(
        output_directory, f"tfrecords_validation_{stamp}"
    ).resolve()
    ...
    checkpoint_filepath = Path(
        output_directory, "weights.{epoch:02d}-t{loss:.2f}-v{val_loss:.2f}.hdf5"
    ).resolve()
...
if __name__ == "__main__":
    ...
    top_dir = Path(
        "results", f'./training-{"%08x" % random.randrange(16**8)}', "%j"
    ).resolve()
            ...
            confs.append(
                {
                    ...
                    "output_directory": Path(
                        top_dir,
                        f"./weights-nFilters{n_filters}-poolSize{pool_size}-checkpoints",
                    ).resolve(),
                    "epochs": args.epochs,
                }
            )
