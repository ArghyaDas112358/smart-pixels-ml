...
def main():
    ...
    p.add_argument('--kl-reverse', action='store_true',
                   help='use reverse KL[student||teacher] (mode-seeking) instead of forward')
    p.add_argument('--fixed-beta', type=float, default=None,
                   help='constant KD weight instead of MDMM lambda')
    ...
    try:
        ...
        distiller = Distiller14(student, teacher,
                                lambda_init=0.0, mdmm_eta=args.mdmm_eta,
                                kl_target=args.kl_target,
                                warmup_steps=args.warmup_steps,
                                kl_reverse=args.kl_reverse,
                                fixed_beta=args.fixed_beta)
        stamp(f"kl_reverse={args.kl_reverse}  fixed_beta={args.fixed_beta}")
