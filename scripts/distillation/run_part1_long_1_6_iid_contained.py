...
1_6 dataset, to watch 2-bit threshold + loss CONVERGENCE over many epochs.

Data: /work/.../shuffled_3d/TFR_files_1_6_iid_contained/{TFR_train,TFR_test}
  - time samples [6,31] (1 ns, 6 ns), N(0,4.64 mV) noise ALREADY BAKED IN  -> load with noise=-1
...
Usage:
  python run_part1_long_1_6_iid_contained.py --seed 42 --epochs 5000
...
TFR = os.path.join(BASE, "TFR_files_1_6_iid_contained")
TFR_TRAIN = os.path.join(TFR, "TFR_train")
TFR_TEST  = os.path.join(TFR, "TFR_test")

OFFSET, LOW, HIGH, TS = 0.0, 25.0, 160.0, [6, 31]
...
def main():
    ...
    ap.add_argument('--out', default='/work/users/das214/SmartPixels/smart-pixels-ml/runs/part1_long_1_6_iid_contained')
    ...
    report_py=os.path.join(os.path.dirname(os.path.abspath(__file__)),'make_1_6_study.py')
