...
from distill14_hint import Distiller14Hint
...
def main():
    ...
    p.add_argument('--teacher-weights-file', default=None,
                   help="single .weights.h5 file for the teacher (overrides --teacher-checkpoints; used for TAKD where the assistant was saved as one file)")
    ...
    p.add_argument('--beta-hint', type=float, default=1.0,
                   help='weight on the feature-matching hint loss')
    ...
    try:
        ...
        if args.teacher_weights_file:
            teacher.load_weights(args.teacher_weights_file)
            stamp(f"teacher loaded from single file: {args.teacher_weights_file}")
        else:
        ...
        distiller = Distiller14Hint(student, teacher,
                                    beta_hint=args.beta_hint,
                                    lambda_init=0.0, mdmm_eta=args.mdmm_eta,
                                    kl_target=args.kl_target,
                                    warmup_steps=args.warmup_steps)
        stamp(f"hint dims: teacher_hidden={distiller._t_dim}  student_hidden={distiller._s_dim}  beta={args.beta_hint}")
        ...
        summary = {
            ...
            'beta_hint': float(args.beta_hint),
            'final_train_hint': float(h.history.get('hint', [float('nan')])[-1]),
            'best_val_hint': float(min(h.history.get('val_hint', [float('inf')]))),
        }
