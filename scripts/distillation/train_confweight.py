...
from distill_confweight import ConfWeightTrainer
...
def main():
    ...
    p.add_argument('--student-model-type', default='QConv2D_Max',
                   choices=['QConv2D_Max', 'Conv2D_Max', 'QConv1D_Full', 'QMlp_Full'])
    ...
    p.add_argument('--clip-ratio', type=float, default=10.0,
                   help='per-event weight clipped to [1/clip_ratio, clip_ratio]')
    ...
    try:
        ...
        distiller = ConfWeightTrainer(student, teacher, clip_ratio=args.clip_ratio)
        ...
        summary = {
            ...
            'final_train_wloss': float(h.history.get('wloss', [float('nan')])[-1]),
            'final_w_spread': float(h.history.get('w_spread', [float('nan')])[-1]),
            'clip_ratio': float(args.clip_ratio),
            'method': 'confidence_weighted_data_NLL (no KL mimicry)',
        }
