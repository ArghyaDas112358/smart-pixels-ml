...
from distill14_means import Distiller14Means
...
def main():
    ...
    try:
        ...
        distiller = Distiller14Means(student, teacher,
        ...
        summary = {
            ...
            'final_train_mse_means': float(h.history.get('mse_means', [float('nan')])[-1]),
            'best_val_mse_means': float(min(h.history.get('val_mse_means', [float('inf')]))),
            'beta': float(args.beta),
            'anneal_steps': float(args.anneal_steps),
            'method': 'means-only MSE distillation',
        }
