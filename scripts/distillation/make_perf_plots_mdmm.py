...
DO NOT point this at a seed that is STILL TRAINING without SMARTPIX_USE_SNAP=1.
HDF5 takes a file lock, and on CephFS reading last.weights.hdf5 while the trainer
rewrites it kills the TRAINER, not the reader:
    BlockingIOError: [Errno 11] Unable to synchronously create file (unable to
    lock file, errno = 11, 'Resource temporarily unavailable')
That is how O13 seed 13042 died at epoch 4871/5000 (2026-08-04). Copy the
checkpoint aside first, or use the SMARTPIX_USE_SNAP path below.
...
def predict_df(weights):
    ...
    # SMARTPIX_DIAG must match the checkpoint's TRAINING dialect (relu for the
    # clip-lineage O11..O21, softplus for loss-v2 O21.v2+). Wrong dialect makes
    # the pulls pure artifact while the resolutions (mu-based) stay fine.
    mn = 1e-9
    _sp = os.environ.get("SMARTPIX_DIAG", "relu") == "softplus"
    for m in ('M11','M22','M33','M44'):
        v = df[m].values
        df[m] = mn + (np.logaddexp(0.0, v) if _sp else np.maximum(v, 0.0))
...
def pull_ax(ax, df, var, name, color='#334155'):
    ...
    try:
        ...
        # Fit stays near-black: it must read against whichever seed hue the
        # histogram is drawn in.
        ax.plot(xb, gauss(xb, *pars), color='#111318', lw=1.1)
    except Exception:
...
def make_pull(df, title, path, color='#334155'):
    fig, ax = plt.subplots(2, 2, sharex=True, figsize=(8, 6))
    fig.suptitle("Pull — " + title, fontsize=10, color=color)
    r = {}
    r['x']    = pull_ax(ax[0][0], df, 'pullx',    r'$x$ pull', color)
    r['y']    = pull_ax(ax[0][1], df, 'pully',    r'$y$ pull', color)
    r['cotA'] = pull_ax(ax[1][0], df, 'pullcotA', r'$\cot\alpha$ pull', color)
    r['cotB'] = pull_ax(ax[1][1], df, 'pullcotB', r'$\cot\beta$ pull', color)
...
def resid_pos(ax, df, v1, v2, name, scaling, color='#334155'):
    a = df[v1].values*scaling; b = df[v2].values*scaling; res = a - b
    nb = 15; xmin, xmax = np.min(a), np.max(a); step = (xmax-xmin)/nb
    sns.regplot(x=a, y=res, x_bins=np.linspace(xmin, xmax, nb), fit_reg=None,
                marker='.', ax=ax, color=color)
    ...
    ax.fill_between(np.linspace(xmin, xmax, nb), up, dn, alpha=0.2, color=color)
    return float(np.std(res))

def resid_ang(ax, df, v1, v2, name, scaling, color='#334155'):
    ...
    res = angT - ang
    sns.regplot(x=angT, y=res, x_bins=np.linspace(xmin, xmax, nb), fit_reg=None,
                marker='.', ax=ax, color=color)
    ax.set_xlabel('True '+name); ax.set_ylabel('True − pred '+name)
    up, dn = [], []
    for i in range(nb):
...
def make_summary(df, title, path, color='#334155'):
    fig, ax = plt.subplots(2, 2, figsize=(9, 7)); fig.tight_layout(pad=4.0)
    fig.suptitle("Summary — " + title, fontsize=10, color=color)
    r = {}
    r['x [um]']     = resid_pos(ax[0][0], df, 'xtrue', 'x', r'$x$ [$\mu$m]', SCALE[0], color)
    r['y [um]']     = resid_pos(ax[0][1], df, 'ytrue', 'y', r'$y$ [$\mu$m]', SCALE[1], color)
    r['alpha[deg]'] = resid_ang(ax[1][0], df, 'cotAtrue', 'cotA', r'$\alpha$ [deg]', SCALE[2], color)
    r['beta[deg]']  = resid_ang(ax[1][1], df, 'cotBtrue', 'cotB', r'$\beta$ [deg]', SCALE[3], color)
...
for seed in SEEDS:
    ...
    tag = (f"seed {seed} @ ep {st['epoch']} — slices {idx}, "
    ...
    # One colour per seed, shared with the history / mu / evolution figures so a
    # seed reads the same everywhere in the deck (see seed_palette.py).
    col = seed_color(seed)
    res = make_summary(df, tag, sp, col)
    pl = make_pull(df, tag, pp, col)
...
# MERGE with whatever is already on disk. Evaluating a subset of a campaign's
# seeds used to overwrite the file, silently dropping the seeds not named in
# THIS invocation -- and the slide scripts read this file, so those seeds just
# vanished from the deck with no error anywhere.
_sp = os.path.join(OUT, "perf_summary_mdmm.json")
_merged = {}
if os.path.exists(_sp):
    try:
        _merged = json.load(open(_sp))
    except (ValueError, OSError):
        _merged = {}
_merged.update(summary)
json.dump(_merged, open(_sp, "w"), indent=2)
