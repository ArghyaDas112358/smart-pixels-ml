"""
ONE long Part-1 run (ViT_Max + SoftQuantize) on the NEW mV baked-noise CONTAINED
2_5 dataset, to watch 2-bit threshold + loss CONVERGENCE over many epochs.

Data: /work/.../shuffled_3d/TFR_files_2_5_noise_corr_contained/{TFR_train,TFR_test}
  - time samples [11,26], N(0,4.64 mV) noise ALREADY BAKED IN  -> load with noise=-1
...
TFR = os.path.join(BASE, "TFR_files_2_5_noise_corr_contained")
...
def main():
    ...
    ap.add_argument('--out', default='/work/users/das214/SmartPixels/smart-pixels-ml/runs/part1_long_2_5_noise_contained')
    ap.add_argument('--seeds', default=None, help='comma-sep seeds to try in order; default = 40 spaced seeds')
    a=ap.parse_args()
    ...
    try:
        ...
        stamp(f"RESEED reseed wrapper: epochs={a.epochs} offset={OFFSET} window[{LOW},{HIGH}] "
              f"GPUs={len(gpus)}  AbortOnStuck(thr=1e4,patience=15,flatline)  ESCAPE_BELOW={ESCAPE_BELOW:g}")
        ...
        for i, seed in enumerate(seeds):
            ...
            if escaped:
                stamp(f"[ESCAPED] seed={seed} after {i} stuck seed(s): best_val={info['best_val_loss']:.1f}"
                      f"@{info['best_epoch']} best_thr={[round(x,2) for x in info['best_thresholds']]} "
                      f"({info['wall_sec']}s) -> runs/.../seed_{seed}")
                json.dump({'winning_seed':seed,'n_stuck_before':i,**info},
                          open(os.path.join(a.out,'WINNER.json'),'w'),indent=1)
                return
            stamp(f"[stuck] seed={seed} aborted={info['aborted_stuck']} best_val={info['best_val_loss']:.1f} "
                  f"(epochs={info['epochs']}) -- trying next seed")
        stamp("EXHAUSTED all seeds without escaping -- investigate (recipe/data issue?)")
    except Exception:
        traceback.print_exc()
